---
description: Pull Linear issues/projects into ~/.leebop cache for editing
argument-hint: [issue-ids…] [--project NAME] [--project-id UUID] [--refresh] [--no-comments] [--to DIR]
---

Run `leebop pull $ARGUMENTS` from the current working directory.
...
2. If the user appears to be starting an edit session, run `leebop status` so they (and you) see baseline cleanliness before any edits.
...
If `leebop` isn't on PATH, stop and report — don't fall back to other tools without asking.
