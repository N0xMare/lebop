...
- [ ] Manual sandbox (lebop-playground/LEB) run if Linear mutation paths, publish/apply behavior, or full-surface release contracts changed (`LEBOP_LIVE_WORKSPACE=lebop-playground LEBOP_LIVE_TEAM=LEB bun scripts/live-nox-surface-smoke.mjs`)
...
- [ ] Validate the generated sandbox live report when the live harness runs (`bun scripts/live-nox-surface-smoke.mjs --validate-report docs/local/live-nox-surface-report-<stamp>.json`)
