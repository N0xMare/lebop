    ...
    process.stdout.write(
      `${JSON.stringify(
        envelope({
          ...
          // Round-8 / R8-LOW-1: surface the staleness-check error string
          // when it errored, so JSON consumers don't have to grep stderr
          // for the prose `note: ...` line that the human mode emits.
          ...(staleErrors.length > 0
            ? { stale_check_error: staleErrors[0]?.error ?? "unknown" }
            : {}),
          clean: {
