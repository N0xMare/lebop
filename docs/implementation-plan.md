...
- **Phase 0 → 4d:** 🟢 shipped and verified.
- **Phase 4d (repo-scoped lint rules):** 🟢 shipped. Three new rules (L004 bracket issue refs, R001 path rewrites, R002 required formats) with full config plumbing — `LintContext` flows through `lintContent` from every command (lint/push/plan apply/plan validate). `applyFixesFixpoint` iterates the lint→fix loop so multiple rules on the same line compose correctly.
- **Next concrete step:** **Phase 4e polish.** Only remaining: **app-actor OAuth** (spec §11). Deferred until audit-trail noise becomes observable.
...
| 4e. Remaining polish | ⬜ | app-actor OAuth (deferred). |
