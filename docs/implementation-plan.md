...
└── lib/
    ...
    ├── errors.ts             # rewriteNotFound: friendly "not found: X" translator
    ├── quirks.ts             # universal Linear renderer rules (L001/L002/L003/L005/L006)
    ├── lint.ts               # rule runner + applyFixes
    ├── planTypes.ts          # plan feature: frontmatter schema + LinkKey mapping
    ├── planParse.ts          # plan feature: directory walker + frontmatter parser
    ├── planValidate.ts       # plan feature: slug/link/cycle/lint/state/label checks
...
- **2026-04-23** — **Phase 4e 🟢 — team-readiness bundle.** Four additions driven by an empirical audit of team UE's Linear usage (150 active issues across 12+ projects): **17/250 sampled issues** use parent/sub-issue (concentrated in `Monorepo MVP`, 17 of its 58); **47/250** use `estimate` (Monorepo MVP heavy adopter + Internal Testnet). Conclusion: both features are real-world patterns we'd hit immediately if leebop were used for a Monorepo-MVP-shaped plan.
  ...
  **Two idempotency bugs fixed during live-verify:**
  - **CREATE path wrote local pre-push body to disk**, not the server-normalized form. Mirrors the earlier fix in `push.ts` (commit `c7d2b23`). Now uses `created.description ?? issue.body`.
  - **`rewriteLinkSlugs` re-read in-memory `issue.body` when re-writing** after slug rewrite, overwriting the just-written normalized body with the stale pre-apply content. Fix: re-read the file from disk before rewriting. Also extended rewriteLinkSlugs to handle `parent:` field (was only rewriting the 5 link fields).

  5 new unit tests in `tests/plan.test.ts` (138 total, all green). Live-verified in sandbox with a 2-issue epic/sub plan + estimates: first apply creates correctly in topological order, parent UUID resolves via look-up, re-apply shows all unchanged, diff clean, remote mutation detected, `plan pull --force` overwrites cleanly.
- **2026-04-23** — **Phase 4d 🟢 — repo-scoped lint rules shipped.** Three new rules per spec §9.2:
  - **L004 — bracket issue refs.** Enabled by `conventions.bracket_issue_refs: true` + `workspaces.<team>.url_prefix`. Flags bare `TEAM-NN` references and autofixes to `[TEAM-NN](<prefix>/issue/TEAM-NN)`. Skips refs already inside markdown links (label OR URL portion) and refs inside code spans.
  - **R001 — path rewrites.** Reads `repos.<path>.path_rewrites: [{from, to}]`. Flags occurrences of `from` that aren't already prefixed with the missing portion of `to`; autofix substitutes. Supports multiple rewrites per repo.
  - **R002 — required formats.** Reads `repos.<path>.required_formats: [{pattern, suggest, message?}]`. Per-match regex rewrite with capture-group interpolation (`$1` etc). Silently skips malformed patterns.

  **Plumbing:** introduced `LintContext { repoConfig, workspaceUrlPrefix }`; updated `Rule.check` signature to take ctx. Universal rules (L001–L003, L005, L006) structurally ignore ctx; repo-scoped rules self-skip when their enabling config is absent. Command-level plumbing touches `lint`, `push`, `plan apply`, `plan validate` — each loads config via `resolveConfig` and builds ctx before calling `lintContent`.
