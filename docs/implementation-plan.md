...
- **Phase 0–3:** 🟢 shipped and verified (see table).
- **Phase 2c (project push live-verify):** 🟢 shipped. Verified against `leebop sandbox` project. Latent post-push hash-drift bug found and fixed: write the server-normalized form back to cache.
- **Phase 3 (linter):** 🟢 shipped — `leebop lint` standalone + `--fix`, `--strict`; integrated into `push` (warn) and `push --strict` (block). Rule catalog: L001 (table cell `N.`), L002 (table cell `- `/`* `), L003 (indented code fence), L005 (URL in backticks), L006 (setext H2 from `text\n---` — discovered during sandbox regression). L004 (bracketed issue refs, repo-scoped) and the `R001`/`R002` repo rules deferred until per-repo config plumbing materializes.
- **Next concrete step:** **Phase 4 polish.** Pick from: slash commands under `~/.claude/commands/`, `SKILL.md` for agent guidance, `leebop diff <id>`, git pre-commit hook on this repo's own fixtures, app-actor OAuth. Or: declarative project-graph (parked idea) — would need its own design pass.
...
| 4. Polish | ⬜ | slash commands, SKILL.md, git pre-commit, `leebop diff`, app-actor OAuth. |
...
Phase 3 is shipped. Remaining surface (Phase 4 polish, in rough value order):

1. **`SKILL.md`** at `~/.claude/skills/leebop/SKILL.md` — when-to-use-which-verb pointer for future agents. Highest-leverage doc for agent users.
2. **Slash commands** under `~/.claude/commands/` (`/leebop-pull`, `/leebop-push`, `/leebop-lint`).
3. **`leebop diff <id>`** — unified diff vs live remote (could become `status --diff <id>` instead of a separate verb).
4. **L004** + repo-scoped `R001`/`R002` linter rules — needs `--strict` plumbing for `bracket_issue_refs` config and `path_rewrites` config.
5. **App-actor OAuth** (spec §11) — only when audit-trail noise becomes observable.
