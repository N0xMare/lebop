...
- **Phase 0 → 4c:** 🟢 shipped and verified.
- **Phase 4b (declarative project-graph):** 🟢 shipped — full `leebop plan` surface: `validate`, `apply`, `diff`, `pull`. Author a project + issue + link graph as markdown files; realize in Linear; detect drift; pull remote changes back into files. Idempotent.
- **Phase 4c (`leebop diff <id>`):** 🟢 shipped — single-issue field + description patch against live remote.
- **Next concrete step:** **Phase 4d polish.** Remaining: L004 + repo-scoped lint rules (`bracket_issue_refs` / `path_rewrites` config plumbing), app-actor OAuth. Neither urgent.
...
| 4d. Remaining polish | ⬜ | L004 + repo-scoped lint rules (needs config plumbing), app-actor OAuth. |
...
Phase 4c shipped. Remaining polish:

1. **L004** + repo-scoped `R001`/`R002` linter rules — needs `bracket_issue_refs` / `path_rewrites` config plumbing.
2. **App-actor OAuth** (spec §11) — only when audit-trail noise becomes observable.
