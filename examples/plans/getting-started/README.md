...
lebop plan validate examples/plans/getting-started

# 3. Preview what `apply` would do.
lebop plan apply examples/plans/getting-started --dry-run

# 4. Apply for real. Creates the project + 3 issues + relations in Linear,
#    then writes `linear_id:` into each frontmatter.
lebop plan apply examples/plans/getting-started
...
grep linear_id examples/plans/getting-started/*.md
